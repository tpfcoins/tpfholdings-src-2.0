...
static std::string ValueToString(int64_t nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::TPF, nValue);
...
static std::string ScriptToString(const CScript& Script, bool Long = false, bool Highlight = false)
{
    ...
    if (ExtractDestination(Script, Dest) && Address.Set(Dest)) {
        if (Highlight)
            ...
            return "<span class=\"addr\">" + Address.ToString() + "</span>"; //return makeHRef(Address.ToString());
    } else
...
static const char* table = "<table border=1>";
...
{
    std::string genesisblockhash = "4fc13b737e2373f19cceb0db121c14e647fc4a350b6d8ecb01c441049dc6d140";
...
std::string BlockToString(CBlockIndex* pBlock)
{
    ...
    std::string TxLabels[] = {_("<strong>Hash</strong>"), _("<strong>From</strong>"), _("<strong>Amount</strong>"), _("<strong>To</strong>"), _("<strong>Amount</strong>")};
    ...
    std::string BlockContentCells[] =
        {
            ...
            _("Transactions "), itostr(block.vtx.size()),
...
*/    
    ...
    std::string Content;
    Content += "<h1><span>" + Address.ToString() + "</span></h1>";
    //Content += TxContent;
    return Content;
}
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", false)) {
            QString Warning = tr("To view all transactions you need to set txindex=1 in the configuration file (tpfcoin.conf).");
            QMessageBox::warning(this, "TPFCoin Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
...
{
    QString CSS = "body {background-color: none;font-size:12px; color:#444;}\n a, span { }\n span.addr { color:#333; }\n table tr td { padding: 5px 8px; border: 1px solid #ccc; background-color: none; }\n td.d0 { }\n h2, h3 { white-space:nowrap; color:#222;}\n a { text-decoration:none; color:#111; }\n";
