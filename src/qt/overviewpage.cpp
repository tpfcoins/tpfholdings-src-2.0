...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::TPF)
    {
    }
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("TPF")
    updateDisplayUnit();
}
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeTPFCoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeTPFCoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeTPFCoinAmount = strAnonymizeTPFCoinAmount.remove(strAnonymizeTPFCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeTPFCoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeTPFCoinAmount * COIN) nMaxToAnonymize = nAnonymizeTPFCoinAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeTPFCoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeTPFCoinAmount));
        strAnonymizeTPFCoinAmount = strAnonymizeTPFCoinAmount.remove(strAnonymizeTPFCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeTPFCoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
