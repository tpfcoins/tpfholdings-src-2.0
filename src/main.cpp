...
#if defined(NDEBUG)
#error "TPFCoin cannot be compiled without assertions."
#endif
...
unsigned int nStakeMinAge = 60 * 60; // 60 min
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight < 29) {
        ...
        nSubsidy = 10000000 * COIN;
    } else if(nHeight > 29 && nHeight < 50000) { 
        nSubsidy = 20 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    if (Params().NetworkID() == CBaseChainParams::TESTNET) {
        if (nHeight < 120){
            ...
            return blockValue * 0.95; //~MN95% - Mining5%
        }
    }
    */
    if (nHeight < 120){
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.IsProofOfWork() && (pindexPrev->nHeight + 1 <= 68589)) {
        ...
        if (abs(n1 - n2) > n1 * 0.5)
            return error("%s : incorrect proof of work (pre-fork) - %f %f %f at %d", __func__, abs(n1 - n2), n1, n2, pindexPrev->nHeight + 1);
