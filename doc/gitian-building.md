...
*Setup instructions for a gitian build of TPFCoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the TPFCoin
...
- [Building TPFCoin](#building-tpfcoin)
...
- The VM will detect network settings using TPFP, this should all proceed automatically
...
Clone the git repositories for tpfcoin and gitian and then checkout the tpfcoin version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/tpfcoin-crypto/tpfcoin.git
cd tpfcoin
...
Follow the instructions in [doc/release-process.md](release-process.md) in the tpfcoin repository
...
To build TPFCoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the tpfcoin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/tpfcoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/tpfcoin-crypto/tpfcoin
...
For example:
```bash
URL=https://github.com/crowning-/tpfcoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit tpfcoin=${COMMIT} --url tpfcoin=${URL} ../tpfcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit tpfcoin=${COMMIT} --url tpfcoin=${URL} ../tpfcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit tpfcoin=${COMMIT} --url tpfcoin=${URL} ../tpfcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/tpfcoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/tpfcoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/tpfcoin-build.assert
```
...
[tpfcoin/gitian.sigs](https://github.com/tpfcoin-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
